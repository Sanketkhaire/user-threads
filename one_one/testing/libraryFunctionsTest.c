...
void thread_create_test(){
    printf(BYELLOW"\n\ttesting one_one thread_create function:\n"NONE);
    ...
    if(create_eflag==0)
        printf(SUCCESS"\tone_one thread_create test passed!\n\n"NONE);
    else
        printf(ERROR"\t\tone_one thread_create test failed!\n\n"NONE);
    return;
}

void thread_join_test(){
    printf(BYELLOW"\n\ttesting one_one thread_join function:\n"NONE);
    ...
    if(create_eflag==0 && join_eflag==0)
        printf(SUCCESS"\tone_one thread_join test passed!\n\n"NONE);
    else
        printf(ERROR"\tone_one thread_join test failed!\n\n"NONE);
    return;
}

void thread_lock_test(){
    printf(BYELLOW"\n\ttesting one_one thread_lock unlock functions:\n"NONE);
    ...
    if(c==c1+c2)
        printf(SUCCESS"\tone_one thread lock test passed\n"NONE);
    else
        printf(ERROR"\tone_one thread_lock test failed!\n\n"NONE);
    return;
}
...
void thread_kill_test(){
    mythread_t tid;
    printf(BYELLOW"\n\ttesting one_one thread_kill function:\n"NONE);
...
void thread_exit_test(){
    printf(BYELLOW"\n\ttesting one_one thread_exit function:\n"NONE);
    ...
    if(create_eflag!=0){
        printf(ERROR"\n\tone_one thread_create test failed!\n"NONE);
        return;
    }
    for(int i=0; i<5; i++){
    ...
    if(exit_flag!=0)
        ...
        printf(SUCCESS"\n\tone_one thread_exit test passed!\n"NONE);
    return;
}

void start_testing(){
    printf(BBLUE"<===================================One-One Library Functions Testing:==============================>\n"NONE);
    return;
}

void stop_testing(){
    printf(BBLUE"<===================================One-One Stress Testing using Matrix Multiplication of higher dimensions:======================>\n"NONE);
    return;
}
